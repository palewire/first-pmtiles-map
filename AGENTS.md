...
This repo builds a PMTiles archive of NOAA IBTrACS tropical cyclone tracks and serves it with MapLibre. It includes a GitHub Pages workflow to publish the static demo page.
...
- `python` (for the local preview server)
...
- `make serve` builds `site/` and serves it at `http://localhost:8000` for quick preview.
...
The Sphinx docs workflow for the tutorial remains separate at [.github/workflows/docs.yaml](.github/workflows/docs.yaml) and continues to build to S3 as configured.
